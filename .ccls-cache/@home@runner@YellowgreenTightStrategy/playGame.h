...
void saveGame(const Player& player, int& highestTreasure){
    ofstream saveFile("save.txt");
    if(saveFile.is_open()){
        saveFile << player.getHealth() << endl;
        saveFile << player.getTreasure() << endl;
        saveFile << highestTreasure << endl;
        saveFile.close();
...
void deleteSaveFile(Player& player) {
    remove("save.txt");

    player.setHealth(100); // Reset player's health to default value
    player.setTreasure(0); // Reset player's treasure to zero
}

void playGame(Player& player, int& treasure, int & highestTreasure){
...
while (true) {
    ...
    cout << "Enter your movement (0 = straight, 1 = left, 2 = right, 3 = quit and save game ): ";
    int action;
    cin >> action;
    ...
    if (action == 0) {
      ...
      std::cout <<"saving and quitting game." << std::endl;
    ...
    cout << ANSI_COLOR_WHITE << "Treasure collected: [" <<  ANSI_COLOR_RESET << ANSI_COLOR_YELLOW << player.getTreasure() << ANSI_COLOR_WHITE "]" ANSI_COLOR_RESET << endl;
       ...
       if (player.getHealth() <= 0) {
           cout << "Your health has reached zero R.I.P" << endl;
          deleteSaveFile(player);
           break; // Break out of the loop and return to the main function
       }
  }
}
#endif 