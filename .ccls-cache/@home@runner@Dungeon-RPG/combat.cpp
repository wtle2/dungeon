...
void Enemy::combat(Player& player) { //combat function to start when player encounters enemy
  
    // Start combat loop
    while (player.getHealth() > 0 && getHealth() > 0) { // while player health is greater than 0 and enemy health is greater than 0 do turn based combat
        // Player's turn
        ...
        std::cin >> choice;
            ...
            srand(static_cast<unsigned>(time(nullptr)));      // optino 3 in which the player can choose to heal themselves
        ...
        if (player.getHealth() <= 0) {
            ...
            break; // Combat ends if player is defeated  
        }
        
        std::cout << ANSI_COLOR_WHITE << "Player Health: [" <<  ANSI_COLOR_RESET << ANSI_COLOR_GREEN << player.getHealth() << ANSI_COLOR_WHITE << "]" <<ANSI_COLOR_RESET << ANSI_COLOR_WHITE << "  Enemy Health: [" << ANSI_COLOR_RESET << ANSI_COLOR_RED << getHealth() << ANSI_COLOR_WHITE << "]" << std::endl; // enemy health
    }
